...
We are lovers of outer space. We follow SpaceX launches, we get excited about real pictures of black holes, and we love to argue over the time scale of the potential heat death of the Universe. We had some questions. Are the distances between all objects growing? Are objects receding from us more quickly as they move further away?
...
How could one make such an observation? We have an answer: Data Science and Machine Learning.
...
**link to data cleaning notebook**
Once we were happy with this, we were ready to create some data frames for our initial analysis. We included the inital pulled data in the file "roughdata" and the final cleaned data in the file "cleandata". We also included the data cleaning process in the note book file "Data_Cleaning.ipynb"
...
- Discover the velocity, flux, and distance for each astronomical object.
- Determine meaningful relationships between variables that are not simply collinear with each other to use for predictive modeling later.
- Obtain an estimate for the Hubble Parameter, the rate of expansion of the universe.
...
**Add Link to project milestone notebook**
